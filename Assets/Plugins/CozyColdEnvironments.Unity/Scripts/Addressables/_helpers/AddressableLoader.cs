    ...
    public static class AddressableLoader
    {
            ...
            Type? assetType = null)
        {
            ...
            return Addressables.ResourceManager.CreateCompletedOperation(
                locations.CastTo<IList<IResourceLocation>>(),
                string.Empty);
            ...
            Addressables.MergeMode mergeMode = Addressables.MergeMode.Intersection)
        {
            ...
            var assetHandles =
                from loc in locationsHandle.Result.AsValueEnumerable()
                select new Prioritized<IResourceLocation, int>(loc, PrioritizedHelper.ResolvePriorityByAttribute(loc.ResourceType, throwIfNotFound: false)) into prioritized
                orderby prioritized
            ...
            return Addressables.ResourceManager.CreateCompletedOperation(
                assets.CastTo<IList<T>>(),
                string.Empty);
        }

        public static async UniTask<AsyncOperationHandle<IList<T>>> LoadPrefabComponentAssetsByLablesAsync<T>(
           string[] labels,
           Action<T>? callback = null,
           Addressables.MergeMode mergeMode = Addressables.MergeMode.Intersection)
        {
