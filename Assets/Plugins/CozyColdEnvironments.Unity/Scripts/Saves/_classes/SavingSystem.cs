    ...
    public sealed partial class SavingSystem : CCBehaviourStaticPublic<SavingSystem>, ISavingSystem
    {
            ...
            CancellationToken cancellationToken)
        {
...
#endif
            {
                ...
                using var regObjs = ListPool<RegisteredObject>.Shared.Get();
                int i = 0;
                ...
                foreach (var regObj in objectSets.Values.SelectMany(x => x))
                {
                    ...
                    regObjs.Value.Add(regObj);
                }

                return regObjs.Value.ToArrayPooled();
            }
#if !PLATFORM_WEBGL
            finally
            {
        ...
        {
            using var keyedSnapshots = ListPool<KeyedSnapshot<ISnapshot>>.Shared.Get();
            using var sceneDatas = ListPool<SaveFileSceneData>.Shared.Get();
            ...
            foreach (var sceneInfo in loadedScenes)
            {
                ...
                foreach (var regObj in regObjects.Value)
                {
                    ...
                    try
                    {
                        ...
                        keyedSnapshots.Value.Add(keyedSnapshot);
                    }
                    catch (Exception ex)
                    {
                ...
                sceneData = new SaveFileSceneData(sceneInfo, keyedSnapshots.Value.ToArray());
                sceneDatas.Value.Add(sceneData);

                keyedSnapshots.Value.Clear();
            }

            return sceneDatas.Value.ToArrayPooled();
